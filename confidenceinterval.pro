pro ConfidenceInterval,name,x_arr,chisq,x_paraname,type_current,x_peak,median_val,lower_val,upper_val,silent=silent
...
;  loadct, 14
  ...
  if not (silent) then plot,x_arr,prob_x_arr,xtitle=x_paraname,ytitle='Posteria PDF',title=string(type_current)

  peaklocation = where(chisq eq min(chisq))
  x_peak = mean(x_arr(peaklocation))
  if not (silent) then print, 'most likely value is', x_peak
  if n_elements(x_peak) gt 5. then stop
  
  cdf_arr = fltarr(n_elements(x_arr))
  for i =1,n_elements(x_arr)-1 do begin
     cdf_arr(i) = int_tabulated(x_arr[0:i],prob_x_arr[0:i])
  endfor
  if not (silent) then plot,x_arr,cdf_arr,ytitle='cdf(x)',xtitle=x_paraname
...
;3) lower bound
  ...
  if x_peak eq 7. then lower_val = 6. ;About IGM abundance at z=2 (O/H)~5.87 Aguirre,2008
  if not (silent) then print, 'the confidence range of x is [',lower_val,',',upper_val,']'  
  ;if x_peak eq 7.893 then stop
  ;wait, 0.5
end
