...
pro metalgradientnew_mar15_fix,angle,xref,yref,xmid,ymid,x,y,n_slits,midpoint,N2map,N2maperr,distance,N2_distance,N2_distance_err,radius,N2_radius,N2_radius_err,slit_size,name,A,sigmaA
    ...
    ;to calculate.
  slitmapfull = readfits('/scr2/nichal/workspace/slitmaps/kineticmodel/'+name+'_slitmap.fits')
  ...
  writefits,'/scr2/nichal/workspace/slitmaps/kineticmodel/'+name+'_slitmap.fits',[[[slitmapfull]],[[N2mapslit]],[[N2map]]]
...
; Now get the N2 index as a function of radius along major axis (N2_radius)
  ...
  for i=0,n_radii-1 do begin
     ...
     if good_radii(0) ne -1 and n_elements(good_radii) gt 1. then begin
        ...
        N2_radius_err(i) = sqrt(total(good_error_sq))/n_elements(good_radii) ;normal error propagation
     endif 
