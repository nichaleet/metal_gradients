...
image =  readfits("/scr2/nichal/workspace/output/abell773_Ha_tlc_Kc3_handmosaic_sky_330hr_acube.fits",header)
scale = 0.1
a773_model,image,header,scale,newimage,newheader,newscale

newimage(where(newimage eq 0.)) = 1./0.
writefits,"/scr2/nichal/workspace/output/abell773_Ha_tlc_Kc3_handmosaic_sky_330hr_acube_sourceplane.fits",newimage,newheader
...
print,'Brightness:', brightness,brightnessnew
print, 'magnification=',(n_elements(goodim)*scale^2)/(n_elements(goodimnew)*newscale^2)

print, 'Now doing Hbeta image '
image =  readfits("/scr2/nichal/workspace/output/abell773_Hb_tlc_Hn3_pipelinemosaic_sky_2hr_acube_aligned.fits",header)
scale = 0.1
a773_model,image,header,scale,newimage,newheader,newscale


newimage(where(newimage eq 0.)) = 1./0.
writefits,"/scr2/nichal/workspace/output/abell773_Hb_tlc_Hn3_pipelinemosaic_sky_2hr_acube_aligned_sourceplane.fits",newimage,newheader
...
image =  readfits("/scr2/nichal/workspace/output/metallicity/Abell773_metallicitynew.fits",header)
scale = 0.1
a773_model,image,header,scale,newimage,newheader,newscale


newimage(where(newimage eq 0.)) = 1./0.
writefits,"/scr2/nichal/workspace/output/metallicity/Abell773_metallicitynew_sourceplane.fits",newimage,newheader
