function lensing_tool_real_color_zoom_hr2(k_dlds,xc,yc,sizec);
...
for m=1:x_size-i_x-5
 for n=1:y_size-5-i_y
   if (BWnew(m,n)>0)
    ...
    im_x=round((m-alpha_x_ALL(m,n)*k)*2-2*750);
    im_y=round((n-alpha_y_ALL(m,n)*k)*2-2*750);  
    if (new_im(m,n,:)>55)
...
for m=1:x_size-i_x-5
   ...
   if (im_x<3000 && im_y< 3000 && im_x>0 && im_y>0)
