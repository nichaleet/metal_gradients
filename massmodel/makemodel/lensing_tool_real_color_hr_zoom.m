function lensing_tool_real_color_hr_zoom(k_dlds,xc,yc,sizec);
...
for m=1:x_size-i_x-5
 for n=1:y_size-5-i_y
   if (BWnew(m,n)>0)
    ...
    im_x=round((m-alpha_x_ALL(m,n)*k)*4-4*750);
    im_y=round((n-alpha_y_ALL(m,n)*k)*4-4*750);  
    if (new_im(m,n,:)>55)
    s_check_realm(im_x,im_y,:)=new_im(m,n,:); 
    source_im(im_x-500,im_y-500,:)=new_im(m,n,:); 
    %marking the source position
    new_im(round((im_x+4*750)/4),round((im_y+4*750)/4),:)=255;
%    if (new_im(m,n,:)>50)
%     round((im_x+4*750)/4)
%    round((im_y+4*750)/4)
    end
