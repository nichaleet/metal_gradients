...
z=1.437
setplot,14
img  =readfits('c142.fits')
gal = img(*,23:37)
bg = [[img(*,0:22)],[img(*,38:60)]]

size=size(gal)
wl=((findgen(size[1])-6.)*1.63+(6562.8*(z+1)))/10000. ;um
yerr = fltarr(size[1])
for i=0, size[1]-1 do yerr(i)=stdev(bg(i,*))
yerr(where(yerr ge 100)) = 350.

;remove one pixel with glitch
gal(where(gal le -120))=0.
...
;stop
plot,wl,total_spec,psym=10,xtitle='um',title='CSWA142'
;fit Halpha
y    = total_spec
yfit = y(5:30)
wlfit= wl(5:30)
...
print,'SN of NII ~ ',sqrt(total(nii))
stop
end
