...
!p.multi = [0,1,2]
restore,'negative_2ndline.sav' ;fitnegativestr
restore,'fitstruct.sav'   ;fitstruct

for i=0, n_elements(fitnegativestr)-1 do begin
NIInow = fitnegativestr(i)
xcoord = NIInow.x
ycoord = NIInow.y
sigma  = NIInow.sigma
if ycoord lt 23 then goto, skip
...
if matchHa(0) eq -1 then stop else begin
   ...
   sigHA = Hanow.sigma
   plot, Hanow.wl,Hanow.data,psym=10,title=string(xcoord)+string(ycoord)+string(sigHa)
   oplot,Hanow.wl,Hanow.fit,color=255
   redshift = Hanow.wl(where(Hanow.fit eq max(Hanow.fit)))/.65628-1.

   plot, NIInow.wl,NIInow.data,psym=10,title=string(xcoord)+string(ycoord)+string(sigma)
