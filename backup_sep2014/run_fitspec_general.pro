...
clean_treshold = 1.
if name eq 'cswa28_Ha_Kn1_mosaic_scaledsky_130hr' then clean_treshold = 0.1
if name eq 'cswa20_Hb_Jn1_mosaic_sky_075hr' then  clean_treshold = 0.1
if name eq 'cswa165_Ha_Kn2_mosaic_sky_1hr' then clean_treshold = 0.5
if name eq 'cswa165_Hb_Hbb_mosaic_scaledsky_1hr' then clean_treshold = 0.5
...
;there is a glitch at x=46 y=32 that lies on top of OIII lines wavelength from pixel 290 to 303
   for wi = 290,303 do incube(wi,46,32)=0.5*(incube(wi,47,32)+incube(wi,45,32))
   for wi = 289,296 do incube(wi,20,28)=0.5*(incube(wi,21,28)+incube(wi,19,28))
   for wi = 297,300 do incube(wi,17,28)=0.5*(incube(wi,16,28)+incube(wi,18,28))
   for wi = 297,301 do incube(wi,17,27)=0.5*(incube(wi,16,27)+incube(wi,18,27))
...
if name eq 'cswa15_Ha_Kn2_mosaic_sky_230hr' then begin
   for wi=152,176 do incube(wi,42,38)=0.5*(incube(wi,41,38)+incube(wi,43,38))
   for wi=146,162 do incube(wi,38,38)=0.5*(incube(wi,37,38)+incube(wi,39,38))
   for wi=144,165 do incube(wi,35,38)=0.5*(incube(wi,34,38)+incube(wi,36,38))
endif 
...
finalcube = incube
wl=wlarr
if name eq 'macs1133_Ha_Hn4_mosaic_scaledsky_and_simplyaddition_230hr' then nmax = 0 else nmax = 0

fitspec_general,finalcube,wl,line,z,sigmalim,xmins,xmaxs,ymins,ymaxs,0,1,nmax,500.,500.,name=name,xmin,xmax,ymin,ymax,header,header_acube,crpix2,crpix3
