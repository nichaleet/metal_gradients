Pro Bayesian_metal,name,OIII,hbeta,NII,halpha,OIII_err,hbeta_err,NII_err,halpha_err,best_metal_arr,median_metal_arr,lowerbound_metal_arr,upperbound_metal_arr,typebayesian
...
;======================================================
metal             = findgen(2500)/1000.+7. ; 12+log(O/H) = 7 to 9.3
x                 = metal-8.69
...
predictedlog_OIII_Hb = 10.^predictedlog_OIII_Hb
...
for ii=0,n_elements(goodpix)-1 do begin
   ...
   if array_equal(typeofchi,[1,1]) then chisq = (NII_Ha_current-predictedlog_NII_Ha)^2/NII_Ha_err_current^2+(OIII_Hb_current-predictedlog_OIII_Hb)^2/OIII_Hb_err_current^2 

   if array_equal(typeofchi,[0,1]) then chisq = (OIII_Hb_current-predictedlog_OIII_Hb)^2/OIII_Hb_err_current^2
      
   if array_equal(typeofchi,[1,0]) then chisq = (NII_Ha_current-predictedlog_NII_Ha)^2/NII_Ha_err_current^2
   if array_equal(typeofchi,[0,0]) then stop
   
   if where(finite(chisq) eq 0) ne [-1.] then stop
   print, 'min chisq is', min(chisq)
   print, 'log[NII]/Ha =',NII_Ha_current
   print, 'log[OIII]/Hb =',OIII_Hb_current
   type_current = Upper_tags(goodpix(ii))+Hbetaupper_tags(goodpix(ii))
   typeBayesian(goodpix(ii)) = type_current
   print, 'type of chisq is', type_current
   ConfidenceInterval,name,metal,chisq,'12+log(O/H)',type_current,x_peak,median_val,lower_val,upper_val
