pro run_fitspec_general_add_noise,input=input,filter=filter,line,z,sigmalim,xmins,xmaxs,ymins,ymaxs,run_fitspec_int_index,secondline,fwhm_gaussian,xmin,xmax,ymin,ymax,crpix2,crpix3
...
;stop
;add noise
noise = randomn(jj(0),size(incube,/dimension))*0.012*(float(jj)*0.1) ;noise for cswa159
plot,incube
incube = incube+noise
oplot,incube,color=255
;pause = get_kbrd()
...
acubefile='/scr2/nichal/workspace/output/addnoise/'+name+'_acube_noise'+strtrim(string(fix(jj)),2)+'.fits'
