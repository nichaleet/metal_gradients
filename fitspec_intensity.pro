...
sigma = output[*,*,2]
odds = exp(sigma^2)
goodsigma = where(odds gt 370. and output[*,*,0] gt 0. and finite(odds)) ;odds of 1 in 371 is for sigma=3 detection
signal = (output[*,*,0])(goodsigma)
print,'At min odds(signal/flat) =370 (3 sigma), detection limit of NII is ', min(signal)
goodsigma = where(odds gt 21. and output[*,*,0] gt 0.) ;odds of 1 in 22 is for sigma=2 detection
signal = (output[*,*,0])(goodsigma)
print,'At min odds(signal/flat) =21 (2 sigma), detection limit of NII is ', min(signal)
goodsigma = where(odds gt 6. and output[*,*,0] gt 0.) ;odds of 1 in 7 is for sigma=1.5 detection
signal = (output[*,*,0])(goodsigma)
print,'At min odds(signal/flat) =6 (1.5 sigma), detection limit of NII is ', min(signal)
stop
end
