...
For ii=0,step_annuli do begin
   ...
   if n_elements(goodpix) gt 1. then begin
      ...
      meanerr,val_radius_pixel(goodpix),val_radius_err_pixel(goodpix),wmean,sigmam,sigmad ;weighted mean
       ...
       print,(ii+0.5)*pixelscale,'kpc :'
       for nn=0,n_elements(goodpix)-1 do print,val_radius_pixel(goodpix(nn)),val_radius_err_pixel(goodpix(nn))
       print, 'mean',wmean,mean(val_radius_pixel(goodpix))
       ;waitt = get_kbrd()
