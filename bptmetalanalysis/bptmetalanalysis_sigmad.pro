...
pro bptmetalanalysis_sigmad,name,path,nii,nii_err,ha,ha_err,oiii,oiii_err,hb,hb_err,angle=angle,xc=xc,yc=yc,pixelscale=pixelscale,slitwidth=slitwidth
;This is the same as the original but use sigmad, standard deviation as an error

outdir = '/scr2/nichal/workspace/output/bptmetalanalysis_sigmad_result/'
...
nii_err_temp(negnii) = nii_temp(negnii)/2. 
n2_temp = nii/ha
...
;do not take the subslit where there is only 1 pixel of h_alpha detection (a lonely island)
         ...
         if n_elements(N2_inslit) eq 1 then N2_radius_err(i) = stdev([N2_inslit,fltarr(n_detects-1)]) else N2_radius_err(i) = stdev(N2_inslit)
      endif else begin
         N2_radius(i) = 1./0.
         N2_radius_err(i) = 1./0.
      endelse
...
if name eq 'cswa19_maingal' then badelements=where(finite(N2_radius) eq 0. or n2_radius lt 0.01)
...
openw, 1,'/scr2/nichal/workspace/output/bptmetalanalysis_sigmad_result/gradient.dat',/append
printf,1, ''
printf,1, name
printf,1, systime()
printf,1, 'N2 PP04'
   ...
   for i=0,n_slits-1 do begin
      ...
      if pix_in_slit(0) ne -1 then begin
         ...
         if thegood(0) ne -1 then begin
            ...
            print,'subslit',i,' O3N2index (simplemean, wmean)=',simplemean,wmean
           ; if abs(simplemean-wmean) gt 30. then stop
            if finite(wmean) eq 1 then begin 
      ...
      if good_radii(0) ne -1 then begin
         ...
         if n_elements(O3N2_inslit) gt 1. then begin
            ...
            if o3n2modified(0) ne -1 then begin
               ...
               O3N2_radius_err(i) = stdev(o3n2_inslit) ;normal error propagation. Because the new uncertainty of the modified one will be smaller than usual. Then the weighted mean will give more faulty weight to these modified ones.
            endif else begin
               meanerr,o3n2_inslit,o3n2err_inslit,wmean,sigmam,sigmad
               o3n2_radius(i) = wmean
               o3n2_radius_err(i) = sigmad
            endelse
         endif else begin
            O3N2_radius(i) = 1./0.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
m08exist = file_search(outdir,+name+'_m08_metal.fits')
if m08exist eq '' then begin
   ...
   writefits,'/scr2/nichal/workspace/output/bptmetalanalysis_sigmad_result/'+name+'_m08_metal.fits',[[[m08metal]],[[median_m08metal]],[[lowerbound_m08metal]],[[upperbound_m08metal]],[[m08metal_err]],[[typebayesian]]]
endif else begin
   m08 = readfits('/scr2/nichal/workspace/output/bptmetalanalysis_sigmad_result/'+name+'_m08_metal.fits')
   m08metal            = m08[*,*,0]
...
for i=0,n_slits-1 do begin
   ...
   if pix_in_slit(0) ne -1 then begin
      ...
      if thegood(0) ne -1 then begin
         ...
         print,'subslit',i,' m08 metal=',simplemean,wmean
         if finite(wmean) eq 1 then begin 
...
x = radius
y = m08_radius
baderr = where(m08_radius_err eq 0.)
if baderr(0) ne -1 then m08_radius_err(baderr) = 1.e-5
